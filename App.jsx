...
const getTideHeight = (dateObj, startYear) => {
    ...
    const H0 = 3.260; // Mean Offset in ft
    const A_M2 = 3.170;
    const phase_M2 = 236.4 * Math.PI / 180;
    const speed_M2 = (2 * Math.PI) / 12.42;

    const A_K1 = 0.319;
    const phase_K1 = 48.4 * Math.PI / 180;
    const speed_K1 = (2 * Math.PI) / 23.93;

    return H0 + A_M2 * Math.cos(speed_M2 * tHours - phase_M2) + A_K1 * Math.cos(speed_K1 * tHours - phase_K1);
};
