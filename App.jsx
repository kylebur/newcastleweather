...
export default function App() {
    ...
    const fetchAllData = useCallback(async () => {
        ...
        setError(null);
        try {
            // Fetch 29 total days: 14 past + today + 14 future
            const lat = 44.0328;
            const lon = -69.5191;
            ...
            const startDateStr = wData.daily.time[0].replace(/-/g, '');
            const endDateStr = wData.daily.time[wData.daily.time.length - 1].replace(/-/g, '');
            ...
            const tideBaseUrl = `https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?begin_date=${startDateStr}&end_date=${endDateStr}&station=8418150&product=predictions&datum=MLLW&time_zone=lst_ldt&units=english&application=Weather_Dash&format=json`;
            
            const [tHiloRes, tContinuousRes] = await Promise.all([
                fetch(`${tideBaseUrl}&interval=hilo`),
                fetch(`${tideBaseUrl}`)
            ]);
            
            const tHiloData = await tHiloRes.json();
            const tContinuousData = await tContinuousRes.json();

            setTideData({
                hilo: tHiloData.predictions || [],
                continuous: tContinuousData.predictions || []
            });
